...
export function useExtension() {
  ...
  const sendToExtension = useCallback((action: string, payload?: any) => {
    ...
    const event = new CustomEvent("MONARCH_WEB_EVENT", {
  ...
  useEffect(() => {
    ...
    // Listen for events
    window.addEventListener("MONARCH_EXT_RESPONSE", handleExtensionMessage);
    ...
    return () => {
      window.removeEventListener("MONARCH_EXT_RESPONSE", handleExtensionMessage);
