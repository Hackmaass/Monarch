...
///      (Monarch backend) may mint new certificates, and each learning session
///      (identified by a unique sessionHash) can only produce one certificate.
contract MonarchCertificate is ERC721, Ownable {
    ...
    constructor(address initialOwner)
        ERC721("Monarch Learning Certificate", "CREDCERT")
        ...
        returns (string memory)
    {
        ...
        string memory json = string(
            abi.encodePacked(
                '{"name":"Monarch Learning Certificate #',
                _toString(tokenId),
                '","description":"Verified proof-of-learning certificate issued by the Monarch platform. '
    ...
    /// @dev "MONARCH" header and "LEARNING CERTIFICATE" subtitle.
    function _svgHeaderText(uint256 tokenId) internal pure returns (string memory) {
        return string(
            abi.encodePacked(
                // Monarch logo text with glow
                ...
                'letter-spacing="12">MONARCH</text>'
                // Subtitle
