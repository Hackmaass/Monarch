...
const MONARCH_CONTRACT_ADDRESS = process.env.PLASMO_PUBLIC_CONTRACT_ADDRESS || "0x98A52554e201DE7aD63E63A7599D0F04D68B3519"; // Placeholder Base Sepolia Address
...
export class UgfBlockchainService {
  ...
  async mintCertificateGasless(
    ...
    const iface = new ethers.Interface(MONARCH_CERTIFICATE_ABI);
  ...
  async getOrCreateSessionWallet(): Promise<ethers.Wallet> {
    const storedKey = localStorage.getItem('monarch_session_key');
    if (storedKey) {
      return new ethers.Wallet(storedKey, this.provider);
    }
    const newWallet = ethers.Wallet.createRandom();
    localStorage.setItem('monarch_session_key', newWallet.privateKey);
