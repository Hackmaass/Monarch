...
console.log("[Monarch] Content script loaded on YouTube")
...
const initTracker = () => {
  ...
  if (!isAllowedChannel(metadata.channelName)) {
    ...
    if (sessionManager) {
      console.log("[Monarch] Leaving non-freeCodeCamp video, ending session")
    ...
    window.dispatchEvent(new CustomEvent("monarch-channel-blocked", {
  ...
  console.log("[Monarch] freeCodeCamp video detected, initializing tracker for:", metadata.title)
  ...
  window.dispatchEvent(new CustomEvent("monarch-session-started", {
...
document.addEventListener("yt-navigate-finish", () => {
  console.log("[Monarch] YouTube navigation detected, re-initializing")
